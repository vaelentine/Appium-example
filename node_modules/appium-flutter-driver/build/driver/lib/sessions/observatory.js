...
// SOCKETS
const connectSocket = async (dartObservatoryURL, RETRY_BACKOFF = 3000, MAX_RETRY_COUNT = 30) => {
    let retryCount = 0;
...
const executeElementCommand = async function (command, elementBase64, extraArgs = {}) {
    const elementObject = elementBase64 ? (0, deserializer_1.deserialize)(elementBase64) : {};
...
const processLogToGetobservatory = (adbLogs) => {
    ...
    const observatoryUriRegEx = new RegExp(`(Observatory listening on |An Observatory debugger and profiler on\\s.+\\sis available at: )((http|\/\/)[a-zA-Z0-9:/=_\\-\.\\[\\]]+)`);
    // @ts-ignore
